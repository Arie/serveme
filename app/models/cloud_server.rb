...
class CloudServer < RemoteServer
  ...
  def mitigation_ssh_exec(command, log_stderr: false)
    ...
    when "remote_docker"
      ...
      out = []
      err = []
      opts = { timeout: 5, keepalive: true, keepalive_interval: 5, keepalive_maxcount: 2, port: docker_host.ssh_port }
  ...
  def sftp_start(&block)
    ...
    Net::SFTP.start(ip, "tf2", port: cloud_ssh_port || 22, key_data: [ cloud_ssh_private_key ], keys_only: true, non_interactive: true, verify_host_key: :never, timeout: 5, config: false, &block)
  end
