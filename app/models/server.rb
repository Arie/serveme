...
class Server < ActiveRecord::Base
  ...
  def update_configuration(reservation)
    ...
    ['reservation.cfg', 'mapconfig/maps/training1.cfg'].each do |config_file|
  ...
  def end_reservation(reservation)
    ...
    disable_plugins
    rcon_exec("sv_logflush 1; tv_stoprecord; sm_kick @all Reservation ended, every player can download the STV demo at http:/​/#{SITE_HOST}")
