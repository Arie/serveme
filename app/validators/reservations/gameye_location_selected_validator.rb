  ...
  class GameyeLocationSelectedValidator < ActiveModel::Validator
    def validate(record)
      if GameyeServer.location_keys.include?(record.gameye_location)
        ...
        record.errors.add(:gameye_location, 'server location is full, please choose another location') if CollisionFinder.new(Reservation.where(gameye_location: location[:id]), record).colliding_reservations.size >= location[:concurrency_limit]
      else
