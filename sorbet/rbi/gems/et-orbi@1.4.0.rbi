# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for types exported from the `et-orbi` gem.
# Please instead update this file by running `bin/tapioca gem et-orbi`.


# pkg:gem/et-orbi#lib/et-orbi/version.rb:2
module EtOrbi
  class << self
    # For `make info`
    #
    # pkg:gem/et-orbi#lib/et-orbi/info.rb:37
    def _make_info; end

    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:89
    def _os_zone; end

    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:89
    def _os_zone=(_arg0); end

    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:109
    def centos_tz; end

    # pkg:gem/et-orbi#lib/et-orbi/make.rb:9
    def chronic_enabled=(b); end

    # pkg:gem/et-orbi#lib/et-orbi/make.rb:6
    def chronic_enabled?; end

    # system tz determination
    #
    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:102
    def debian_tz; end

    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:53
    def determine_local_tzone; end

    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:35
    def extract_zone(str); end

    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:131
    def gather_tzs; end

    # pkg:gem/et-orbi#lib/et-orbi/zone.rb:6
    def get_tzone(o); end

    # https://en.wikipedia.org/wiki/ISO_8601
    # Postel's law applies
    #
    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:21
    def list_iso8601_zones(s); end

    # pkg:gem/et-orbi#lib/et-orbi/make.rb:69
    def make(*a); end

    # pkg:gem/et-orbi#lib/et-orbi/make.rb:51
    def make_time(*a); end

    # pkg:gem/et-orbi#lib/et-orbi/make.rb:15
    def now(zone = T.unsafe(nil)); end

    # test tool
    #
    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:91
    def os_tz; end

    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:122
    def osx_tz; end

    # pkg:gem/et-orbi#lib/et-orbi/make.rb:20
    def parse(str, opts = T.unsafe(nil)); end

    # pkg:gem/et-orbi#lib/et-orbi/info.rb:6
    def platform_info; end

    # pkg:gem/et-orbi#lib/et-orbi/info.rb:43
    def render_nozone_time(seconds); end

    # pkg:gem/et-orbi#lib/et-orbi/time.rb:507
    def rweek_ref; end

    # pkg:gem/et-orbi#lib/et-orbi/time.rb:495
    def rweek_ref=(x); end

    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:176
    def tweak_zone_name(name); end

    # Semi-helpful, since it requires the current time
    #
    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:138
    def windows_zone_name(zone_name, time); end

    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:87
    def zone; end

    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:164
    def zone_abbreviation(zone_name, time); end

    protected

    # pkg:gem/et-orbi#lib/et-orbi/make.rb:73
    def chronic_parse(str, opts); end

    # TZInfo < 2.0.0
    # TZInfo >= 2.0.0
    #
    # pkg:gem/et-orbi#lib/et-orbi/zone.rb:56
    def create_offset_tzone(utc_off, id); end

    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:252
    def custom_tzs; end

    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:222
    def determine_local_tzones; end

    # https://api.rubyonrails.org/classes/ActiveSupport/TimeWithZone.html
    #
    # If it responds to #time_zone, then return that time zone.
    #
    # pkg:gem/et-orbi#lib/et-orbi/zone.rb:126
    def get_as_tzone(t); end

    # pkg:gem/et-orbi#lib/et-orbi/zone.rb:115
    def get_local_tzone(t); end

    # custom timezones, no DST, just an offset, like "+08:00" or "-01:30"
    #
    # pkg:gem/et-orbi#lib/et-orbi/zone.rb:29
    def get_offset_tzone(str); end

    # pkg:gem/et-orbi#lib/et-orbi/zone.rb:95
    def get_tzinfo_tzone(name); end

    # pkg:gem/et-orbi#lib/et-orbi/zone.rb:74
    def get_x_offset_tzone(str); end

    # pkg:gem/et-orbi#lib/et-orbi/make.rb:108
    def make_from_array(a, zone); end

    # pkg:gem/et-orbi#lib/et-orbi/make.rb:99
    def make_from_date(d, zone); end

    # pkg:gem/et-orbi#lib/et-orbi/make.rb:125
    def make_from_eotime(eot, zone); end

    # pkg:gem/et-orbi#lib/et-orbi/make.rb:120
    def make_from_numeric(f, zone); end

    # pkg:gem/et-orbi#lib/et-orbi/make.rb:115
    def make_from_string(s, zone); end

    # pkg:gem/et-orbi#lib/et-orbi/make.rb:84
    def make_from_time(t, zone); end

    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:188
    def normalize(name); end

    # pkg:gem/et-orbi#lib/et-orbi/zone.rb:84
    def to_offset(n); end

    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:253
    def tz_all; end

    # pkg:gem/et-orbi#lib/et-orbi/info.rb:68
    def tzinfo_data_version; end

    # pkg:gem/et-orbi#lib/et-orbi/info.rb:60
    def tzinfo_version; end

    # pkg:gem/et-orbi#lib/et-orbi/zones.rb:193
    def unzz(name); end

    # pkg:gem/et-orbi#lib/et-orbi/zone.rb:106
    def windows_zone_code_x(zone_name); end
  end
end

# Our EoTime class (which quacks like a ::Time).
#
# An EoTime instance should respond to most of the methods ::Time instances
# respond to. If a method is missing, feel free to open an issue to
# ask (politely) for it. If it makes sense, it'll get added, else
# a workaround will get suggested.
# The immediate workaround is to call #to_t on the EoTime instance to get
# equivalent ::Time instance in the local, current, timezone.
#
# pkg:gem/et-orbi#lib/et-orbi/time.rb:13
class EtOrbi::EoTime
  # pkg:gem/et-orbi#lib/et-orbi/time.rb:67
  def initialize(s, zone); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:250
  def +(t); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:251
  def -(t); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:243
  def <(o); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:244
  def <=(o); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:245
  def <=>(o); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:220
  def ==(o); end

  # Nota Bene:
  #
  # Unlike ==, the equal? method should never be overridden by subclasses
  # as it is used to determine object identity (that is, a.equal?(b) if and
  # only if a is the same object as b)
  #
  # The eql? method returns true if obj and other refer to the same hash key.
  # This is used by Hash to test members for equality.
  #
  # pkg:gem/et-orbi#lib/et-orbi/time.rb:241
  def >(o); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:242
  def >=(o); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:247
  def add(t); end

  # Returns true if this EoTime instance corresponds to 2 different UTC
  # times.
  # It happens when transitioning from DST to winter time.
  #
  # https://www.timeanddate.com/time/change/usa/new-york?year=2018
  #
  # pkg:gem/et-orbi#lib/et-orbi/time.rb:127
  def ambiguous?; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:217
  def asctime; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:413
  def clone; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:217
  def day; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:156
  def getgm; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:155
  def getutc; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:217
  def hour; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:352
  def in_time_zone(zone = T.unsafe(nil)); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:316
  def inc(t, dir = T.unsafe(nil)); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:188
  def is_dst?; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:192
  def isdst; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:291
  def iso8601(fraction_digits = T.unsafe(nil)); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:346
  def localtime(zone = T.unsafe(nil)); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:217
  def min; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:217
  def month; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:256
  def monthdays; end

  # "reference day", used in fugit for cron modulo notation
  #
  # pkg:gem/et-orbi#lib/et-orbi/time.rb:362
  def rday; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:381
  def reach(points); end

  # A debug method, seen in its first iteration in
  # https://github.com/floraison/et-orbi?tab=readme-ov-file#rweek-and-rday-clarification-et-orbi-140
  #
  # pkg:gem/et-orbi#lib/et-orbi/time.rb:379
  def rr; end

  # "reference week", used in fugit for cron modulo notation
  #
  # pkg:gem/et-orbi#lib/et-orbi/time.rb:371
  def rweek; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:217
  def sec; end

  # instance methods
  #
  # pkg:gem/et-orbi#lib/et-orbi/time.rb:64
  def seconds; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:102
  def seconds=(f); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:169
  def strftime(format); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:248
  def subtract(t); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:194
  def to_debug_s; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:159
  def to_f; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:164
  def to_i; end

  # Returns this ::EtOrbi::EoTime as a ::Time instance
  # in the current timezone.
  #
  # Has a #to_t alias.
  #
  # pkg:gem/et-orbi#lib/et-orbi/time.rb:181
  def to_local_time; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:281
  def to_s; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:186
  def to_t; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:311
  def to_time_s; end

  # Debug current time by showing local time / delta / utc time
  # for example: "0120-7(0820)"
  #
  # pkg:gem/et-orbi#lib/et-orbi/time.rb:300
  def to_utc_comparison_s; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:157
  def to_utc_time; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:286
  def to_zs; end

  # Nullify the "caches" used by #to_time, #rday, and others
  #
  # pkg:gem/et-orbi#lib/et-orbi/time.rb:96
  def touch; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:351
  def translate(zone = T.unsafe(nil)); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:217
  def usec; end

  # Returns this ::EtOrbi::EoTime as a ::Time instance
  # in the current UTC timezone.
  #
  # pkg:gem/et-orbi#lib/et-orbi/time.rb:141
  def utc; end

  # Returns true if this ::EtOrbi::EoTime instance timezone is UTC.
  # Returns false else.
  #
  # pkg:gem/et-orbi#lib/et-orbi/time.rb:149
  def utc?; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:209
  def utc_offset; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:217
  def wday; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:354
  def wday_in_month; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:217
  def yday; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:217
  def year; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:65
  def zone; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:111
  def zone=(z); end

  protected

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:469
  def _to_f(o); end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:430
  def count_weeks(start, dir); end

  # Gives back the Eotime instance pointing to the reference day
  # used to compute #rweek and #rday
  #
  # pkg:gem/et-orbi#lib/et-orbi/time.rb:481
  def rref; end

  # pkg:gem/et-orbi#lib/et-orbi/time.rb:443
  def strfz(code); end

  # Returns a Ruby Time instance.
  #
  # Warning: the timezone of that Time instance will be UTC when used with
  # TZInfo < 2.0.0.
  #
  # pkg:gem/et-orbi#lib/et-orbi/time.rb:425
  def to_time; end

  class << self
    # pkg:gem/et-orbi#lib/et-orbi/time.rb:30
    def get_tzone(o); end

    # pkg:gem/et-orbi#lib/et-orbi/time.rb:55
    def local(*a); end

    # pkg:gem/et-orbi#lib/et-orbi/time.rb:35
    def local_tzone; end

    # pkg:gem/et-orbi#lib/et-orbi/time.rb:45
    def make(o); end

    # pkg:gem/et-orbi#lib/et-orbi/time.rb:20
    def now(zone = T.unsafe(nil)); end

    # pkg:gem/et-orbi#lib/et-orbi/time.rb:25
    def parse(str, opts = T.unsafe(nil)); end

    # pkg:gem/et-orbi#lib/et-orbi/time.rb:40
    def platform_info; end

    # pkg:gem/et-orbi#lib/et-orbi/time.rb:50
    def utc(*a); end
  end
end

# pkg:gem/et-orbi#lib/et-orbi/time.rb:253
EtOrbi::EoTime::DAY_S = T.let(T.unsafe(nil), Integer)

# pkg:gem/et-orbi#lib/et-orbi/time.rb:254
EtOrbi::EoTime::WEEK_S = T.let(T.unsafe(nil), Integer)

# RWEEK_REF_DEFAULT = '2019-01-01'.freeze
#
# pkg:gem/et-orbi#lib/et-orbi/time.rb:491
EtOrbi::RWEEK_REF_DEFAULT = T.let(T.unsafe(nil), String)

# pkg:gem/et-orbi#lib/et-orbi/version.rb:4
EtOrbi::VERSION = T.let(T.unsafe(nil), String)

# https://docs.microsoft.com/en-us/windows-hardware/manufacture/desktop/default-time-zones
# https://support.microsoft.com/en-ca/help/973627/microsoft-time-zone-index-values
# https://ss64.com/nt/timezones.html
#
# pkg:gem/et-orbi#lib/et-orbi/zones.rb:260
EtOrbi::ZONE_ALIASES = T.let(T.unsafe(nil), Hash)
