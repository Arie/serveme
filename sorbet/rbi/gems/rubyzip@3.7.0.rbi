# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for types exported from the `rubyzip` gem.
# Please instead update this file by running `bin/tapioca gem rubyzip`.


# Rubyzip is a ruby module for reading and writing zip files.
#
# The main entry points are File, InputStream and OutputStream. For a
# file/directory interface in the style of the standard ruby ::File and
# ::Dir APIs then `require 'zip/filesystem'` and see FileSystem.
#
# pkg:gem/rubyzip#lib/zip/version.rb:3
module Zip
  extend ::Zip

  # pkg:gem/rubyzip#lib/zip.rb:48
  def case_insensitive_match; end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def case_insensitive_match=(_arg0); end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def continue_on_exists_proc; end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def continue_on_exists_proc=(_arg0); end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def default_compression; end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def default_compression=(_arg0); end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def force_entry_names_encoding; end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def force_entry_names_encoding=(_arg0); end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def on_exists_proc; end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def on_exists_proc=(_arg0); end

  # pkg:gem/rubyzip#lib/zip.rb:70
  def reset!; end

  # Set options for RubyZip in one block.
  #
  # pkg:gem/rubyzip#lib/zip.rb:86
  def setup; end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def sort_entries; end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def sort_entries=(_arg0); end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def unicode_names; end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def unicode_names=(_arg0); end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def validate_declared_number_of_entries; end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def validate_declared_number_of_entries=(_arg0); end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def validate_entry_sizes; end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def validate_entry_sizes=(_arg0); end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def warn_invalid_date; end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def warn_invalid_date=(_arg0); end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def write_zip64_support; end

  # pkg:gem/rubyzip#lib/zip.rb:48
  def write_zip64_support=(_arg0); end
end

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:168
class Zip::AESDecrypter < ::Zip::Decrypter
  include ::Zip::AESEncryption

  # pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:208
  def check_integrity!(io); end

  # pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:171
  def decrypt(encrypted_data); end

  # pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:194
  def reset!(header); end
end

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:91
class Zip::AESEncrypter < ::Zip::Encrypter
  include ::Zip::AESEncryption

  # pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:123
  def crc(_computed_crc); end

  # pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:113
  def data_descriptor(*); end

  # `Deflater`/`PassThruCompressor` call this with whatever, arbitrarily
  # sized (and not necessarily block-aligned) buffer they happen to have
  # flushed, potentially many times per entry. Only whole 16-byte blocks
  # are actually run through the cipher here; any trailing partial block
  # is buffered in `@pending` until either more data completes it or
  # `trailer` forces the final flush. This keeps the CTR counter aligned
  # with the true byte offset in the plaintext stream regardless of how
  # callers happen to chunk their writes.
  #
  # pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:106
  def encrypt(data); end

  # pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:94
  def header(_mtime); end

  # pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:139
  def prepare_entry(entry); end

  # pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:127
  def reset!; end

  # pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:117
  def trailer; end

  private

  # pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:145
  def encrypt_blocks(final: T.unsafe(nil)); end
end

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:6
module Zip::AESEncryption
  # pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:47
  def initialize(password, strength); end

  # pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:64
  def gp_flags; end

  # pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:60
  def header_bytesize; end

  private

  # Derive the encryption key, HMAC key and password-verification value
  # from the password and a salt, as specified by the WinZip AES format.
  #
  # pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:72
  def derive_keys(salt); end
end

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:9
Zip::AESEncryption::AUTHENTICATION_CODE_LENGTH = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:29
Zip::AESEncryption::BITS = T.let(T.unsafe(nil), Hash)

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:8
Zip::AESEncryption::BLOCK_SIZE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:35
Zip::AESEncryption::KEY_LENGTHS = T.let(T.unsafe(nil), Hash)

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:41
Zip::AESEncryption::SALT_LENGTHS = T.let(T.unsafe(nil), Hash)

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:23
Zip::AESEncryption::STRENGTHS = T.let(T.unsafe(nil), Array)

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:19
Zip::AESEncryption::STRENGTH_128_BIT = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:20
Zip::AESEncryption::STRENGTH_192_BIT = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:21
Zip::AESEncryption::STRENGTH_256_BIT = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:7
Zip::AESEncryption::VERIFIER_LENGTH = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:14
Zip::AESEncryption::VERSIONS = T.let(T.unsafe(nil), Array)

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:11
Zip::AESEncryption::VERSION_AE_1 = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/crypto/aes_encryption.rb:12
Zip::AESEncryption::VERSION_AE_2 = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:9
Zip::CDIR_ENTRY_STATIC_HEADER_LENGTH = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:8
Zip::CENTRAL_DIRECTORY_ENTRY_SIGNATURE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:96
Zip::COMPRESSION_METHODS = T.let(T.unsafe(nil), Hash)

# pkg:gem/rubyzip#lib/zip/constants.rb:94
Zip::COMPRESSION_METHOD_AES = T.let(T.unsafe(nil), Integer)

# RESERVED = 11
#
# pkg:gem/rubyzip#lib/zip/constants.rb:83
Zip::COMPRESSION_METHOD_BZIP2 = T.let(T.unsafe(nil), Integer)

# RESERVED = 7
#
# pkg:gem/rubyzip#lib/zip/constants.rb:79
Zip::COMPRESSION_METHOD_DEFLATE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:80
Zip::COMPRESSION_METHOD_DEFLATE_64 = T.let(T.unsafe(nil), Integer)

# RESERVED = 15
#
# pkg:gem/rubyzip#lib/zip/constants.rb:87
Zip::COMPRESSION_METHOD_IBM_CMPSC = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:90
Zip::COMPRESSION_METHOD_IBM_LZ77 = T.let(T.unsafe(nil), Integer)

# RESERVED = 17
#
# pkg:gem/rubyzip#lib/zip/constants.rb:89
Zip::COMPRESSION_METHOD_IBM_TERSE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:77
Zip::COMPRESSION_METHOD_IMPLODE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:91
Zip::COMPRESSION_METHOD_JPEG = T.let(T.unsafe(nil), Integer)

# RESERVED = 13
#
# pkg:gem/rubyzip#lib/zip/constants.rb:85
Zip::COMPRESSION_METHOD_LZMA = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:81
Zip::COMPRESSION_METHOD_PKWARE_DCLI = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:93
Zip::COMPRESSION_METHOD_PPMD = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:73
Zip::COMPRESSION_METHOD_REDUCE_1 = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:74
Zip::COMPRESSION_METHOD_REDUCE_2 = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:75
Zip::COMPRESSION_METHOD_REDUCE_3 = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:76
Zip::COMPRESSION_METHOD_REDUCE_4 = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:72
Zip::COMPRESSION_METHOD_SHRINK = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:71
Zip::COMPRESSION_METHOD_STORE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:92
Zip::COMPRESSION_METHOD_WAVPACK = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/central_directory.rb:8
class Zip::CentralDirectory
  include ::Zip::Dirtyable
  extend ::Forwardable
  extend ::Zip::Dirtyable::ClassMethods

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:31
  def initialize(entries = T.unsafe(nil), comment = T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:25
  def <<(param); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:66
  def ==(other); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:23
  def comment; end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:23
  def comment=(param); end

  # Reads the End of Central Directory Record (and the Zip64 equivalent if
  # needs be) and returns the number of entries in the archive. This is a
  # convenience method that avoids reading in all of the entry data to get a
  # very quick entry count.
  #
  # pkg:gem/rubyzip#lib/zip/central_directory.rb:61
  def count_entries(io); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:25
  def delete(param); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:25
  def each(*, **, &); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:25
  def entries(*, **, &); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:25
  def find_entry(*, **, &); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:25
  def glob(*, **, &); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:25
  def include?(*, **, &); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:29
  def orig_comment=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:29
  def orig_delete(*, **, &); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:37
  def read_from_stream(io); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:25
  def size(*, **, &); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:42
  def write_to_stream(io, suppress_extra_fields: T.unsafe(nil)); end

  private

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:258
  def eocd_data(io); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:175
  def read_central_directory_entries(io); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:219
  def read_eocds(io); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:208
  def read_local_extra_field(io); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:115
  def unpack_64_e_o_c_d(buffer); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:141
  def unpack_64_eocd_locator(buffer); end

  # Unpack the EOCD and return a boolean indicating whether this header is
  # complete without needing Zip64 extensions.
  #
  # pkg:gem/rubyzip#lib/zip/central_directory.rb:150
  def unpack_e_o_c_d(buffer); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:268
  def validate_size!(size, size_in_bytes); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:89
  def write_64_e_o_c_d(io, offset, cdir_size); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:105
  def write_64_eocd_locator(io, zip64_eocd_offset); end

  # pkg:gem/rubyzip#lib/zip/central_directory.rb:74
  def write_e_o_c_d(io, offset, cdir_size); end
end

# pkg:gem/rubyzip#lib/zip/central_directory.rb:12
Zip::CentralDirectory::END_OF_CD_SIG = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/central_directory.rb:20
Zip::CentralDirectory::MAX_END_OF_CD_SIZE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/central_directory.rb:19
Zip::CentralDirectory::MAX_FILE_COMMENT_SIZE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/central_directory.rb:16
Zip::CentralDirectory::STATIC_EOCD_SIZE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/central_directory.rb:13
Zip::CentralDirectory::ZIP64_END_OF_CD_SIG = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/central_directory.rb:14
Zip::CentralDirectory::ZIP64_EOCD_LOCATOR_SIG = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/central_directory.rb:18
Zip::CentralDirectory::ZIP64_EOCD_LOC_SIZE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/central_directory.rb:17
Zip::CentralDirectory::ZIP64_STATIC_EOCD_SIZE = T.let(T.unsafe(nil), Integer)

# Error raised if an unsupported compression method is used.
#
# pkg:gem/rubyzip#lib/zip/errors.rb:9
class Zip::CompressionMethodError < ::Zip::Error
  # Create a new CompressionMethodError with the specified incorrect
  # compression method.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:15
  def initialize(method); end

  # The compression method that has caused this error.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:11
  def compression_method; end

  # The message returned by this error.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:21
  def message; end
end

# pkg:gem/rubyzip#lib/zip/compressor.rb:4
class Zip::Compressor
  # pkg:gem/rubyzip#lib/zip/compressor.rb:5
  def finish; end
end

# pkg:gem/rubyzip#lib/zip.rb:60
Zip::DEFAULT_RESTORE_OPTIONS = T.let(T.unsafe(nil), Hash)

# pkg:gem/rubyzip#lib/zip/dos_time.rb:6
class Zip::DOSTime < ::Time
  include ::Zip::DOSTime::JRubyCMP

  # Dos time is only stored with two seconds accuracy.
  #
  # pkg:gem/rubyzip#lib/zip/dos_time.rb:64
  def <=>(other); end

  # 23:59:58
  #
  # pkg:gem/rubyzip#lib/zip/dos_time.rb:31
  def absolute_time=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/dos_time.rb:33
  def absolute_time?; end

  # Deprecated. Remove for version 4.
  #
  # pkg:gem/rubyzip#lib/zip/dos_time.rb:58
  def dos_equals(other); end

  # pkg:gem/rubyzip#lib/zip/dos_time.rb:48
  def to_binary_dos_date; end

  # pkg:gem/rubyzip#lib/zip/dos_time.rb:39
  def to_binary_dos_time; end

  class << self
    # Create a DOSTime instance from a vanilla Time instance.
    #
    # pkg:gem/rubyzip#lib/zip/dos_time.rb:71
    def from_time(time); end

    # pkg:gem/rubyzip#lib/zip/dos_time.rb:75
    def parse_binary_dos_format(bin_dos_date, bin_dos_time); end
  end
end

# The MS-DOS date field is seven bits of "year minus 1980", so only
# 1980-01-01 00:00:00 through 2107-12-31 23:59:58 can be represented.
# Times outside that are clamped rather than allowed to wrap: the real
# time is still carried in the universal time (UT) extra field, but the
# DOS fields have to stay in range for readers that only look at those.
#
# pkg:gem/rubyzip#lib/zip/dos_time.rb:24
Zip::DOSTime::DOS_EPOCH_YEAR = T.let(T.unsafe(nil), Integer)

# 1980-01-01
#
# pkg:gem/rubyzip#lib/zip/dos_time.rb:27
Zip::DOSTime::DOS_MAX_DATE = T.let(T.unsafe(nil), Integer)

# 00:00:00
#
# pkg:gem/rubyzip#lib/zip/dos_time.rb:29
Zip::DOSTime::DOS_MAX_TIME = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/dos_time.rb:25
Zip::DOSTime::DOS_MAX_YEAR = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/dos_time.rb:26
Zip::DOSTime::DOS_MIN_DATE = T.let(T.unsafe(nil), Integer)

# 2107-12-31
#
# pkg:gem/rubyzip#lib/zip/dos_time.rb:28
Zip::DOSTime::DOS_MIN_TIME = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/dos_time.rb:89
module Zip::DOSTime::JRubyCMP
  # pkg:gem/rubyzip#lib/zip/dos_time.rb:94
  def <(other); end

  # pkg:gem/rubyzip#lib/zip/dos_time.rb:98
  def <=(other); end

  # pkg:gem/rubyzip#lib/zip/dos_time.rb:90
  def ==(other); end

  # pkg:gem/rubyzip#lib/zip/dos_time.rb:102
  def >(other); end

  # pkg:gem/rubyzip#lib/zip/dos_time.rb:106
  def >=(other); end
end

# Error raised if there is a problem while decompressing an archive entry.
#
# pkg:gem/rubyzip#lib/zip/errors.rb:27
class Zip::DecompressionError < ::Zip::Error
  # Create a new DecompressionError with the specified underlying Zlib
  # error.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:33
  def initialize(zlib_error); end

  # The message returned by this error.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:39
  def message; end

  # The error from the underlying Zlib library that caused this error.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:29
  def zlib_error; end
end

# pkg:gem/rubyzip#lib/zip/decompressor.rb:4
class Zip::Decompressor
  # pkg:gem/rubyzip#lib/zip/decompressor.rb:21
  def initialize(input_stream, decompressed_size = T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/decompressor.rb:19
  def decompressed_size; end

  # pkg:gem/rubyzip#lib/zip/decompressor.rb:19
  def input_stream; end

  class << self
    # pkg:gem/rubyzip#lib/zip/decompressor.rb:7
    def decompressor_classes; end

    # pkg:gem/rubyzip#lib/zip/decompressor.rb:15
    def find_by_compression_method(compression_method); end

    # pkg:gem/rubyzip#lib/zip/decompressor.rb:11
    def register(compression_method, decompressor_class); end
  end
end

# pkg:gem/rubyzip#lib/zip/decompressor.rb:5
Zip::Decompressor::CHUNK_SIZE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/crypto/decrypted_io.rb:4
class Zip::DecryptedIo
  # pkg:gem/rubyzip#lib/zip/crypto/decrypted_io.rb:7
  def initialize(io, decrypter, compressed_size); end

  # pkg:gem/rubyzip#lib/zip/crypto/decrypted_io.rb:17
  def read(maxlen = T.unsafe(nil)); end

  private

  # pkg:gem/rubyzip#lib/zip/crypto/decrypted_io.rb:36
  def eof?; end

  # pkg:gem/rubyzip#lib/zip/crypto/decrypted_io.rb:40
  def input_finished?; end

  # pkg:gem/rubyzip#lib/zip/crypto/decrypted_io.rb:44
  def produce_input; end
end

# pkg:gem/rubyzip#lib/zip/crypto/decrypted_io.rb:5
Zip::DecryptedIo::CHUNK_SIZE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/crypto/encryption.rb:16
class Zip::Decrypter; end

# pkg:gem/rubyzip#lib/zip/deflater.rb:4
class Zip::Deflater < ::Zip::Compressor
  # pkg:gem/rubyzip#lib/zip/deflater.rb:5
  def initialize(output_stream, level = T.unsafe(nil), encrypter = T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/deflater.rb:14
  def <<(data); end

  # pkg:gem/rubyzip#lib/zip/deflater.rb:32
  def crc; end

  # pkg:gem/rubyzip#lib/zip/deflater.rb:26
  def finish; end

  # pkg:gem/rubyzip#lib/zip/deflater.rb:32
  def size; end
end

# Error raised when trying to extract an archive entry over an
# existing file.
#
# pkg:gem/rubyzip#lib/zip/errors.rb:46
class Zip::DestinationExistsError < ::Zip::Error
  # Create a new DestinationExistsError with the clashing destination.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:48
  def initialize(destination); end

  # The message returned by this error.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:54
  def message; end
end

# pkg:gem/rubyzip#lib/zip/dirtyable.rb:4
module Zip::Dirtyable
  mixes_in_class_methods ::Zip::Dirtyable::ClassMethods

  # pkg:gem/rubyzip#lib/zip/dirtyable.rb:5
  def initialize(dirty_on_create: T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/dirtyable.rb:9
  def dirty?; end

  class << self
    # pkg:gem/rubyzip#lib/zip/dirtyable.rb:28
    def included(base); end
  end
end

# pkg:gem/rubyzip#lib/zip/dirtyable.rb:13
module Zip::Dirtyable::ClassMethods
  # pkg:gem/rubyzip#lib/zip/dirtyable.rb:14
  def mark_dirty(*symbols); end
end

# pkg:gem/rubyzip#lib/zip/crypto/encryption.rb:4
class Zip::Encrypter
  # pkg:gem/rubyzip#lib/zip/crypto/encryption.rb:9
  def crc(computed_crc); end

  # pkg:gem/rubyzip#lib/zip/crypto/encryption.rb:13
  def prepare_entry(_entry); end

  # pkg:gem/rubyzip#lib/zip/crypto/encryption.rb:5
  def trailer; end
end

# Zip::Entry represents an entry in a Zip archive.
#
# pkg:gem/rubyzip#lib/zip/entry.rb:10
class Zip::Entry
  include ::Zip::Dirtyable
  extend ::Zip::Dirtyable::ClassMethods

  # Create a new Zip::Entry.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:76
  def initialize(zipfile = T.unsafe(nil), name = T.unsafe(nil), comment: T.unsafe(nil), size: T.unsafe(nil), compressed_size: T.unsafe(nil), crc: T.unsafe(nil), compression_method: T.unsafe(nil), compression_level: T.unsafe(nil), time: T.unsafe(nil), extra: T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:662
  def <=>(other); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:653
  def ==(other); end

  # Does this entry return time fields with accurate timezone information?
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:181
  def absolute_time?; end

  # Is this entry encrypted with AES encryption?
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:207
  def aes?; end

  # Get the last access time of this entry, if available.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:144
  def atime; end

  # Set the last access time of this entry.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:171
  def atime=(value); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:274
  def calculate_local_header_size; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:290
  def cdir_header_size; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:518
  def check_c_dir_entry_comment_size; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:512
  def check_c_dir_entry_signature; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:506
  def check_c_dir_entry_static_header_length(buf); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:70
  def check_name(name); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:757
  def clean_up; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def comment; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def comment=(param); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:270
  def comment_size; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def compressed_size; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def compressed_size=(param); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:34
  def compression_level; end

  # Return the compression method for this entry.
  #
  # Returns STORED if the entry is a directory or if the compression
  # level is 0.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:189
  def compression_method; end

  # Set the compression method for this entry.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:196
  def compression_method=(method); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:31
  def crc; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:31
  def crc=(_arg0); end

  # Get the creation time of this entry, if available.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:149
  def ctime; end

  # Set the creation time of this entry.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:176
  def ctime=(value); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:236
  def directory?; end

  # Is this entry encrypted?
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:112
  def encrypted?; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:31
  def external_file_attributes; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:31
  def external_file_attributes=(param); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:34
  def extra; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:266
  def extra_size; end

  # Extracts this entry to a file at `entry_path`, with
  # `destination_directory` as the base location in the filesystem.
  #
  # NB: The caller is responsible for making sure `destination_directory` is
  # safe, if it is passed.
  #
  # `create_parent_directories`, if true, creates any missing intermediate
  # directories for a file entry before writing it. This defaults to
  # false: a missing intermediate directory usually means an earlier
  # entry (e.g. a symlink) was deliberately skipped as unsafe, and letting
  # a later entry silently succeed by re-creating that path as a plain
  # directory would mask that. Bulk operations such as `extract_all`,
  # which have no such earlier-entry to skip past, opt into this.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:312
  def extract(entry_path = T.unsafe(nil), destination_directory: T.unsafe(nil), create_parent_directories: T.unsafe(nil), &block); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:236
  def file?; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:554
  def file_stat(path); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:226
  def file_type_is?(type); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:34
  def filepath; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def follow_symlinks; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def follow_symlinks=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:31
  def fstype; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:31
  def fstype=(param); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:230
  def ftype; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:701
  def gather_fileinfo_from_srcpath(src_path); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:562
  def get_extra_attributes_from_path(path); end

  # Returns an IO like object for the given ZipEntry.
  # Optional decrypter can be provided for an encrypted entry
  # Warning: may behave weird with symlinks.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:669
  def get_input_stream(decrypter: T.unsafe(nil), &block); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:749
  def get_raw_input_stream(&block); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:31
  def gp_flags; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:31
  def gp_flags=(param); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:116
  def incomplete?; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:31
  def internal_file_attributes; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:31
  def internal_file_attributes=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:258
  def local_entry_offset; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:31
  def local_header_offset; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:31
  def local_header_offset=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:141
  def mtime(component: T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:168
  def mtime=(value, component: T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def name; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def name=(param); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:241
  def name_is_directory?; end

  # Is the name a relative path, free of `..` patterns that could lead to
  # path traversal attacks? This does NOT handle symlinks; if the path
  # contains symlinks, this check is NOT enough to guarantee safety.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:248
  def name_safe?; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:262
  def name_size; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:295
  def next_header_offset; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:38
  def orig_comment=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:38
  def orig_compressed_size=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:38
  def orig_external_file_attributes=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:38
  def orig_fstype=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:38
  def orig_gp_flags=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:38
  def orig_name=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:38
  def orig_size=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:38
  def orig_unix_gid=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:38
  def orig_unix_perms=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:38
  def orig_unix_uid=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:599
  def pack_c_dir_entry; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:420
  def pack_local_entry; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:743
  def parent_as_string; end

  # Called by `AESEncrypter#prepare_entry` before the local header is first
  # written, so the extra field is in place before its size is measured.
  # Directories have no content to encrypt, so they are left untouched.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:214
  def prep_aes_extra(vendor_version, strength); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:532
  def read_c_dir_entry(io); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:524
  def read_extra_field(buf, local: T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:376
  def read_local_entry(io); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def restore_ownership; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def restore_ownership=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def restore_permissions; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def restore_permissions=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def restore_times; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def restore_times=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:42
  def set_default_vars_values; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:585
  def set_extra_attributes_on_path(dest_path); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:476
  def set_ftype_from_c_dir_entry; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:573
  def set_unix_attributes_on_path(dest_path); end

  # The uncompressed size of the entry.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:121
  def size; end

  # The uncompressed size of the entry.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:36
  def size=(param); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:236
  def symlink?; end

  # Get a timestamp component of this entry.
  #
  # Returns modification time by default.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:128
  def time(component: T.unsafe(nil)); end

  # Set a timestamp component of this entry.
  #
  # Sets modification time by default.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:156
  def time=(value, component: T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:332
  def to_s; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def unix_gid; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def unix_gid=(param); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def unix_perms; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def unix_perms=(param); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def unix_uid; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:27
  def unix_uid=(param); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:455
  def unpack_c_dir_entry(buf); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:361
  def unpack_local_entry(buf); end

  # check before rewriting an entry (after file sizes are known)
  # that we didn't change the header size (and thus clobber file data or something)
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:280
  def verify_local_header_size!; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:623
  def write_c_dir_entry(io, suppress_extra_fields: T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:435
  def write_local_entry(io, suppress_extra_fields: T.unsafe(nil), rewrite: T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:730
  def write_to_zip_output_stream(zip_output_stream); end

  # Does this entry use the ZIP64 extensions?
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:202
  def zip64?; end

  private

  # pkg:gem/rubyzip#lib/zip/entry.rb:805
  def create_directory(dest_path); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:779
  def create_file(dest_path, _continue_on_exists_proc = T.unsafe(nil)); end

  # BUG: create_symlink() does not use &block
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:819
  def create_symlink(dest_path); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:839
  def parse_aes_extra; end

  # apply missing data from the zip64 extra information field, if present
  # (required when file sizes exceed 2**32, but can be used for all files)
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:827
  def parse_zip64_extra(for_local_header); end

  # pkg:gem/rubyzip#lib/zip/entry.rb:899
  def prep_cdir_zip64_extra; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:882
  def prep_local_zip64_extra; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:763
  def read_local_header_fields(io); end

  # For DEFLATED compression *only*: set the general purpose flags 1 and 2 to
  # indicate compression level. This seems to be mainly cosmetic but they are
  # generally set by other tools - including in docx files. It is these flags
  # that are used by commandline tools (and elsewhere) to give an indication
  # of how compressed a file is. See the PKWARE APPNOTE for more information:
  # https://pkware.cachefly.net/webdocs/casestudies/APPNOTE.TXT
  #
  # It's safe to simply OR these flags here as compression_level is read only.
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:868
  def set_compression_level_flags; end

  # pkg:gem/rubyzip#lib/zip/entry.rb:773
  def set_time(binary_dos_date, binary_dos_time); end

  # AES-encrypted entries always store `COMPRESSION_METHOD_AES` (99) on the
  # wire; the real compression method lives in the AES extra field instead
  # (see `parse_aes_extra`, which reverses this on read).
  #
  # pkg:gem/rubyzip#lib/zip/entry.rb:856
  def stored_compression_method; end

  class << self
    # pkg:gem/rubyzip#lib/zip/entry.rb:337
    def read_c_dir_entry(io); end

    # pkg:gem/rubyzip#lib/zip/entry.rb:350
    def read_local_entry(io); end
  end
end

# pkg:gem/rubyzip#lib/zip/entry.rb:24
Zip::Entry::COMPRESSION_LEVEL_FAST_GPFLAG = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/entry.rb:25
Zip::Entry::COMPRESSION_LEVEL_MAX_GPFLAG = T.let(T.unsafe(nil), Integer)

# Compression level flags (used as part of the gp flags).
#
# pkg:gem/rubyzip#lib/zip/entry.rb:23
Zip::Entry::COMPRESSION_LEVEL_SUPERFAST_GPFLAG = T.let(T.unsafe(nil), Integer)

# Constant used to specify that the entry is deflated (i.e., compressed).
#
# pkg:gem/rubyzip#lib/zip/entry.rb:17
Zip::Entry::DEFLATED = T.let(T.unsafe(nil), Integer)

# Language encoding flag (EFS) bit
#
# pkg:gem/rubyzip#lib/zip/entry.rb:20
Zip::Entry::EFS = T.let(T.unsafe(nil), Integer)

# Constant used to specify that the entry is stored (i.e., not compressed).
#
# pkg:gem/rubyzip#lib/zip/entry.rb:14
Zip::Entry::STORED = T.let(T.unsafe(nil), Integer)

# Error raised when trying to add an entry to an archive where the
# entry name already exists.
#
# pkg:gem/rubyzip#lib/zip/errors.rb:62
class Zip::EntryExistsError < ::Zip::Error
  # Create a new EntryExistsError with the specified source and name.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:64
  def initialize(source, name); end

  # The message returned by this error.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:71
  def message; end
end

# Error raised when an entry name is invalid.
#
# pkg:gem/rubyzip#lib/zip/errors.rb:77
class Zip::EntryNameError < ::Zip::Error
  # Create a new EntryNameError with the specified name.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:79
  def initialize(name = T.unsafe(nil)); end

  # The message returned by this error.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:85
  def message; end
end

# pkg:gem/rubyzip#lib/zip/errors.rb:112
class Zip::EntryNumberMismatchError < ::Zip::Error
  # Create a new EntryNumberMismatchError with the specified size and size in bytes.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:114
  def initialize(size, size_in_bytes); end

  # The message returned by this error.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:121
  def message; end
end

# pkg:gem/rubyzip#lib/zip/entry_set.rb:4
class Zip::EntrySet
  include ::Enumerable

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:10
  def initialize(an_enumerable = T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:24
  def <<(entry); end

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:53
  def ==(other); end

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:36
  def delete(entry); end

  # deep clone
  #
  # pkg:gem/rubyzip#lib/zip/entry_set.rb:49
  def dup; end

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:40
  def each(&block); end

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:44
  def entries; end

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:20
  def find_entry(entry); end

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:63
  def glob(pattern, flags = T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:16
  def include?(entry); end

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:34
  def length; end

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:59
  def parent(entry); end

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:28
  def push(entry); end

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:30
  def size; end

  protected

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:7
  def entry_set; end

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:74
  def sorted_entries; end

  private

  # pkg:gem/rubyzip#lib/zip/entry_set.rb:80
  def to_key(entry); end
end

# Error raised if an entry is larger on extraction than it is advertised
# to be.
#
# pkg:gem/rubyzip#lib/zip/errors.rb:96
class Zip::EntrySizeError < ::Zip::Error
  # Create a new EntrySizeError with the specified entry.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:101
  def initialize(entry); end

  # The entry that has caused this error.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:98
  def entry; end

  # The message returned by this error.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:107
  def message; end
end

# The superclass for all rubyzip error types. Simply rescue this one if
# you don't need to know what sort of error has been raised.
#
# pkg:gem/rubyzip#lib/zip/errors.rb:6
class Zip::Error < ::StandardError; end

# pkg:gem/rubyzip#lib/zip/extra_field.rb:4
class Zip::ExtraField < ::Hash
  # pkg:gem/rubyzip#lib/zip/extra_field.rb:7
  def initialize(binstr = T.unsafe(nil), local: T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/extra_field.rb:81
  def c_dir_size; end

  # pkg:gem/rubyzip#lib/zip/extra_field.rb:48
  def create(name); end

  # pkg:gem/rubyzip#lib/zip/extra_field.rb:11
  def extra_field_type_exist(binstr, id, len, index); end

  # pkg:gem/rubyzip#lib/zip/extra_field.rb:21
  def extra_field_type_unknown(binstr, len, index, local); end

  # pkg:gem/rubyzip#lib/zip/extra_field.rb:89
  def length; end

  # pkg:gem/rubyzip#lib/zip/extra_field.rb:85
  def local_size; end

  # pkg:gem/rubyzip#lib/zip/extra_field.rb:32
  def merge(binstr, local: T.unsafe(nil)); end

  # Place Unknown last, so "extra" data that is missing the proper
  # signature/size does not prevent known fields from being read back in.
  #
  # pkg:gem/rubyzip#lib/zip/extra_field.rb:58
  def ordered_values; end

  # pkg:gem/rubyzip#lib/zip/extra_field.rb:90
  def size; end

  # Remove any extra fields that indicate they can be safely suppressed.
  #
  # pkg:gem/rubyzip#lib/zip/extra_field.rb:65
  def suppress_fields!(fields); end

  # pkg:gem/rubyzip#lib/zip/extra_field.rb:77
  def to_c_dir_bin; end

  # pkg:gem/rubyzip#lib/zip/extra_field.rb:71
  def to_local_bin; end

  # pkg:gem/rubyzip#lib/zip/extra_field.rb:75
  def to_s; end
end

# Info-ZIP Extra for AES encryption
#
# pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:5
class Zip::ExtraField::AES < ::Zip::ExtraField::Generic
  # pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:11
  def initialize(binstr = T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:19
  def ==(other); end

  # pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:6
  def compression_method; end

  # pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:6
  def compression_method=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:6
  def encryption_strength; end

  # pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:6
  def encryption_strength=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:26
  def merge(binstr); end

  # pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:46
  def pack_for_c_dir; end

  # pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:41
  def pack_for_local; end

  # We can never suppress the AES extra field as it is needed to read the file.
  #
  # pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:37
  def suppress?; end

  # pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:6
  def vendor_id; end

  # pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:6
  def vendor_id=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:6
  def vendor_version; end

  # pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:6
  def vendor_version=(_arg0); end
end

# pkg:gem/rubyzip#lib/zip/extra_field/aes.rb:8
Zip::ExtraField::AES::HEADER_ID = T.let(T.unsafe(nil), String)

# pkg:gem/rubyzip#lib/zip/extra_field/generic.rb:4
class Zip::ExtraField::Generic
  # return field [size, content] or false
  #
  # pkg:gem/rubyzip#lib/zip/extra_field/generic.rb:16
  def initial_parse(binstr); end

  # Default strategy is to suppress all extra fields if we're asked to.
  # Specific extra field types can override this if they need to be kept.
  #
  # pkg:gem/rubyzip#lib/zip/extra_field/generic.rb:29
  def suppress?; end

  # pkg:gem/rubyzip#lib/zip/extra_field/generic.rb:38
  def to_c_dir_bin; end

  # pkg:gem/rubyzip#lib/zip/extra_field/generic.rb:33
  def to_local_bin; end

  class << self
    # pkg:gem/rubyzip#lib/zip/extra_field/generic.rb:11
    def name; end

    # pkg:gem/rubyzip#lib/zip/extra_field/generic.rb:5
    def register_map; end
  end
end

# pkg:gem/rubyzip#lib/zip/extra_field.rb:5
Zip::ExtraField::ID_MAP = T.let(T.unsafe(nil), Hash)

# Info-ZIP Extra for UNIX uid/gid
#
# pkg:gem/rubyzip#lib/zip/extra_field/unix.rb:5
class Zip::ExtraField::IUnix < ::Zip::ExtraField::Generic
  # pkg:gem/rubyzip#lib/zip/extra_field/unix.rb:9
  def initialize(binstr = T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/extra_field/unix.rb:29
  def ==(other); end

  # pkg:gem/rubyzip#lib/zip/extra_field/unix.rb:15
  def gid; end

  # pkg:gem/rubyzip#lib/zip/extra_field/unix.rb:15
  def gid=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/extra_field/unix.rb:17
  def merge(binstr); end

  # pkg:gem/rubyzip#lib/zip/extra_field/unix.rb:37
  def pack_for_c_dir; end

  # pkg:gem/rubyzip#lib/zip/extra_field/unix.rb:33
  def pack_for_local; end

  # pkg:gem/rubyzip#lib/zip/extra_field/unix.rb:15
  def uid; end

  # pkg:gem/rubyzip#lib/zip/extra_field/unix.rb:15
  def uid=(_arg0); end
end

# pkg:gem/rubyzip#lib/zip/extra_field/unix.rb:6
Zip::ExtraField::IUnix::HEADER_ID = T.let(T.unsafe(nil), String)

# PKWARE NTFS Extra Field (0x000a)
# Only Tag 0x0001 is supported
#
# pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:6
class Zip::ExtraField::NTFS < ::Zip::ExtraField::Generic
  # pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:13
  def initialize(binstr = T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:40
  def ==(other); end

  # pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:20
  def atime; end

  # pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:20
  def atime=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:20
  def ctime; end

  # pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:20
  def ctime=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:22
  def merge(binstr); end

  # pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:20
  def mtime; end

  # pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:20
  def mtime=(_arg0); end

  # But 7-zip for Windows only stores at central dir
  #
  # pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:52
  def pack_for_c_dir; end

  # Info-ZIP note states this extra field is stored at local header
  #
  # pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:47
  def pack_for_local; end

  private

  # pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:88
  def from_ntfs_time(ntfs_time); end

  # pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:70
  def parse_tags(content); end

  # pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:92
  def to_ntfs_time(time); end
end

# pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:7
Zip::ExtraField::NTFS::HEADER_ID = T.let(T.unsafe(nil), String)

# pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:11
Zip::ExtraField::NTFS::SEC_TO_UNIX_EPOCH = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/extra_field/ntfs.rb:10
Zip::ExtraField::NTFS::WINDOWS_TICK = T.let(T.unsafe(nil), Float)

# Olf Info-ZIP Extra for UNIX uid/gid and file timestampes
#
# pkg:gem/rubyzip#lib/zip/extra_field/old_unix.rb:5
class Zip::ExtraField::OldUnix < ::Zip::ExtraField::Generic
  # pkg:gem/rubyzip#lib/zip/extra_field/old_unix.rb:9
  def initialize(binstr = T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/extra_field/old_unix.rb:33
  def ==(other); end

  # pkg:gem/rubyzip#lib/zip/extra_field/old_unix.rb:17
  def atime; end

  # pkg:gem/rubyzip#lib/zip/extra_field/old_unix.rb:17
  def atime=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/extra_field/old_unix.rb:17
  def gid; end

  # pkg:gem/rubyzip#lib/zip/extra_field/old_unix.rb:17
  def gid=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/extra_field/old_unix.rb:19
  def merge(binstr); end

  # pkg:gem/rubyzip#lib/zip/extra_field/old_unix.rb:17
  def mtime; end

  # pkg:gem/rubyzip#lib/zip/extra_field/old_unix.rb:17
  def mtime=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/extra_field/old_unix.rb:44
  def pack_for_c_dir; end

  # pkg:gem/rubyzip#lib/zip/extra_field/old_unix.rb:40
  def pack_for_local; end

  # pkg:gem/rubyzip#lib/zip/extra_field/old_unix.rb:17
  def uid; end

  # pkg:gem/rubyzip#lib/zip/extra_field/old_unix.rb:17
  def uid=(_arg0); end
end

# pkg:gem/rubyzip#lib/zip/extra_field/old_unix.rb:6
Zip::ExtraField::OldUnix::HEADER_ID = T.let(T.unsafe(nil), String)

# Info-ZIP Additional timestamp field
#
# pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:5
class Zip::ExtraField::UniversalTime < ::Zip::ExtraField::Generic
  # pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:13
  def initialize(binstr = T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:59
  def ==(other); end

  # pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:22
  def atime; end

  # pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:24
  def atime=(time); end

  # pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:22
  def ctime; end

  # pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:29
  def ctime=(time); end

  # pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:22
  def flag; end

  # pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:39
  def merge(binstr); end

  # pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:22
  def mtime; end

  # pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:34
  def mtime=(time); end

  # pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:73
  def pack_for_c_dir; end

  # pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:65
  def pack_for_local; end
end

# pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:9
Zip::ExtraField::UniversalTime::ATIME_MASK = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:10
Zip::ExtraField::UniversalTime::CTIME_MASK = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:6
Zip::ExtraField::UniversalTime::HEADER_ID = T.let(T.unsafe(nil), String)

# pkg:gem/rubyzip#lib/zip/extra_field/universal_time.rb:11
Zip::ExtraField::UniversalTime::MTIME_MASK = T.let(T.unsafe(nil), Integer)

# A class to hold unknown extra fields so that they are preserved.
#
# pkg:gem/rubyzip#lib/zip/extra_field/unknown.rb:7
class Zip::ExtraField::Unknown < ::Zip::ExtraField::Generic
  # pkg:gem/rubyzip#lib/zip/extra_field/unknown.rb:8
  def initialize; end

  # pkg:gem/rubyzip#lib/zip/extra_field/unknown.rb:31
  def ==(other); end

  # pkg:gem/rubyzip#lib/zip/extra_field/unknown.rb:13
  def merge(binstr, local: T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/extra_field/unknown.rb:27
  def to_c_dir_bin; end

  # pkg:gem/rubyzip#lib/zip/extra_field/unknown.rb:23
  def to_local_bin; end
end

# Info-ZIP Extra for Zip64 size
#
# pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:5
class Zip::ExtraField::Zip64 < ::Zip::ExtraField::Generic
  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:12
  def initialize(binstr = T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:24
  def ==(other); end

  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:6
  def compressed_size; end

  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:6
  def compressed_size=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:6
  def disk_start_number; end

  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:6
  def disk_start_number=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:31
  def merge(binstr); end

  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:6
  def original_size; end

  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:6
  def original_size=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:74
  def pack_for_c_dir; end

  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:66
  def pack_for_local; end

  # pass the values from the base entry (if applicable)
  # wider values are only present in the extra field for base values set to all FFs
  # returns the final values for the four attributes (from the base or zip64 extra record)
  #
  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:40
  def parse(original_size, compressed_size, relative_header_offset = T.unsafe(nil), disk_start_number = T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:6
  def relative_header_offset; end

  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:6
  def relative_header_offset=(_arg0); end

  # We can suppress the zip64 extra field unless we know the size is large or
  # the relative header offset is large (for central directory entries).
  #
  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:61
  def suppress?; end

  private

  # pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:54
  def extract(size, format); end
end

# pkg:gem/rubyzip#lib/zip/extra_field/zip64.rb:9
Zip::ExtraField::Zip64::HEADER_ID = T.let(T.unsafe(nil), String)

# pkg:gem/rubyzip#lib/zip/constants.rb:22
Zip::FILE_TYPE_DIR = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:21
Zip::FILE_TYPE_FILE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:23
Zip::FILE_TYPE_SYMLINK = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:47
Zip::FSTYPES = T.let(T.unsafe(nil), Hash)

# pkg:gem/rubyzip#lib/zip/constants.rb:38
Zip::FSTYPE_ACORN = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:26
Zip::FSTYPE_AMIGA = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:30
Zip::FSTYPE_ATARI = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:45
Zip::FSTYPE_ATHEOS = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:41
Zip::FSTYPE_BEOS = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:34
Zip::FSTYPE_CPM = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:25
Zip::FSTYPE_FAT = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:31
Zip::FSTYPE_HPFS = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:32
Zip::FSTYPE_MAC = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:44
Zip::FSTYPE_MAC_OSX = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:40
Zip::FSTYPE_MVS = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:36
Zip::FSTYPE_NTFS = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:37
Zip::FSTYPE_QDOS = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:42
Zip::FSTYPE_TANDEM = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:43
Zip::FSTYPE_THEOS = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:35
Zip::FSTYPE_TOPS20 = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:28
Zip::FSTYPE_UNIX = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:39
Zip::FSTYPE_VFAT = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:27
Zip::FSTYPE_VMS = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:29
Zip::FSTYPE_VM_CMS = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:33
Zip::FSTYPE_Z_SYSTEM = T.let(T.unsafe(nil), Integer)

# Zip::File is modeled after java.util.zip.ZipFile from the Java SDK.
# The most important methods are those for accessing information about
# the entries in
# the archive and methods such as `get_input_stream` and
# `get_output_stream` for reading from and writing entries to the
# archive. The class includes a few convenience methods such as
# `extract` for extracting entries to the filesystem, `extract_all` for
# extracting every entry, `add_recursive` for adding the contents of a
# directory tree, and `remove`, `replace`, `rename` and `mkdir` for
# making simple modifications to the archive.
#
# Modifications to a zip archive are not committed until `commit` or
# `close` is called. The method `open` accepts a block following
# the pattern from ::File.open offering a simple way to
# automatically close the archive when the block returns.
#
# The following example opens zip archive `my.zip`
# (creating it if it doesn't exist) and adds an entry
# `first.txt` and a directory entry `a_dir`
# to it.
#
# ```
# require 'zip'
#
# Zip::File.open('my.zip', create: true) do |zipfile|
#   zipfile.get_output_stream('first.txt') { |f| f.puts 'Hello from Zip::File' }
#   zipfile.mkdir('a_dir')
# end
# ```
#
# The next example reopens `my.zip`, writes the contents of
# `first.txt` to standard out and deletes the entry from
# the archive.
#
# ```
# require 'zip'
#
# Zip::File.open('my.zip', create: true) do |zipfile|
#   puts zipfile.read('first.txt')
#   zipfile.remove('first.txt')
# end
#
# Zip::FileSystem offers an alternative API that emulates ruby's
# interface for accessing the filesystem, ie. the ::File and ::Dir classes.
#
# pkg:gem/rubyzip#lib/zip/file.rb:53
class Zip::File
  include ::Enumerable
  extend ::Forwardable
  extend ::Zip::FileSplit

  # Opens a zip archive. Pass create: true to create
  # a new archive if it doesn't exist already.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:76
  def initialize(path_or_io, create: T.unsafe(nil), buffer: T.unsafe(nil), restore_ownership: T.unsafe(nil), restore_permissions: T.unsafe(nil), restore_times: T.unsafe(nil), compression_level: T.unsafe(nil), suppress_extra_fields: T.unsafe(nil)); end

  # Convenience method for adding the contents of a file to the archive
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:254
  def add(entry, src_path, &continue_on_exists_proc); end

  # Recursively adds the contents of `src_dir` to the archive, nested
  # under `prefix` if given (the archive root otherwise). `src_dir`
  # itself is not added, only its contents.
  #
  # Symlinks are ignored (skipped, with a warning) rather than followed
  # or added, to avoid the security issues they can pose. `max_depth`
  # limits how many directory levels below `src_dir` are walked; anything
  # deeper is skipped, also with a warning (default: 16).
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:286
  def add_recursive(src_dir, prefix: T.unsafe(nil), max_depth: T.unsafe(nil), &continue_on_exists_proc); end

  # Convenience method for adding the contents of a file to the archive
  # in Stored format (uncompressed)
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:271
  def add_stored(entry, src_path, &continue_on_exists_proc); end

  # Closes the zip file committing any changes that has been made.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:387
  def close; end

  # pkg:gem/rubyzip#lib/zip/file.rb:72
  def comment(*, **, &); end

  # pkg:gem/rubyzip#lib/zip/file.rb:72
  def comment=(*, **, &); end

  # Commits changes that has been made since the previous commit to
  # the zip archive.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:360
  def commit; end

  # Returns true if any changes has been made to this archive since
  # the previous commit
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:393
  def commit_required?; end

  # pkg:gem/rubyzip#lib/zip/file.rb:72
  def each(*, **, &); end

  # pkg:gem/rubyzip#lib/zip/file.rb:72
  def entries(*, **, &); end

  # Extracts `entry` to a file at `entry_path`, with `destination_directory`
  # as the base location in the filesystem.
  #
  # NB: The caller is responsible for making sure `destination_directory` is
  # safe, if it is passed.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:324
  def extract(entry, entry_path = T.unsafe(nil), destination_directory: T.unsafe(nil), &block); end

  # Extracts every entry in the archive into `destination_directory`,
  # preserving the archive's directory structure.
  #
  # Symlink entries are ignored (skipped, with a warning) rather than
  # extracted. See `Entry#extract` for the path-safety checks applied to
  # every other entry.
  #
  # NB: The caller is responsible for making sure `destination_directory` is
  # safe, if it is passed.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:340
  def extract_all(destination_directory = T.unsafe(nil), &block); end

  # Searches for entry with the specified name. Returns nil if
  # no entry is found. See also get_entry
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:405
  def find_entry(entry_name); end

  # Searches for an entry just as find_entry, but throws Errno::ENOENT
  # if no entry is found.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:417
  def get_entry(entry); end

  # Returns an input stream to the specified entry. If a block is passed
  # the stream object is passed to the block and the stream is automatically
  # closed afterwards just as with ruby's builtin File.open method.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:209
  def get_input_stream(entry, &a_proc); end

  # Returns an output stream to the specified entry. If entry is not an instance
  # of Zip::Entry, a new Zip::Entry will be initialized using the arguments
  # specified. If a block is passed the stream object is passed to the block and
  # the stream is automatically closed afterwards just as with ruby's builtin
  # File.open method.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:218
  def get_output_stream(entry, permissions: T.unsafe(nil), comment: T.unsafe(nil), extra: T.unsafe(nil), compressed_size: T.unsafe(nil), crc: T.unsafe(nil), compression_method: T.unsafe(nil), compression_level: T.unsafe(nil), size: T.unsafe(nil), time: T.unsafe(nil), &a_proc); end

  # pkg:gem/rubyzip#lib/zip/file.rb:72
  def glob(*, **, &); end

  # Creates a directory
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:425
  def mkdir(entry_name, permission = T.unsafe(nil)); end

  # The name of this zip archive.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:61
  def name; end

  # Returns a string containing the contents of the specified entry
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:249
  def read(entry); end

  # Removes the specified entry.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:298
  def remove(entry); end

  # Renames the specified entry.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:303
  def rename(entry, new_name, &continue_on_exists_proc); end

  # Replaces the specified entry with the contents of src_path (from
  # the file system).
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:313
  def replace(entry, src_path); end

  # default -> false.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:64
  def restore_ownership; end

  # default -> false.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:64
  def restore_ownership=(_arg0); end

  # default -> true.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:67
  def restore_permissions; end

  # default -> true.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:67
  def restore_permissions=(_arg0); end

  # default -> true.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:70
  def restore_times; end

  # default -> true.
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:70
  def restore_times=(_arg0); end

  # pkg:gem/rubyzip#lib/zip/file.rb:72
  def size(*, **, &); end

  # Returns the name of the zip archive
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:244
  def to_s; end

  # Write buffer write changes to buffer and return
  #
  # pkg:gem/rubyzip#lib/zip/file.rb:377
  def write_buffer(io = T.unsafe(nil)); end

  private

  # pkg:gem/rubyzip#lib/zip/file.rb:435
  def add_recursive_dir(entry_prefix, src_dir, depth, max_depth, continue_on_exists_proc); end

  # pkg:gem/rubyzip#lib/zip/file.rb:490
  def check_entry_exists(entry_name, continue_on_exists_proc, proc_name); end

  # pkg:gem/rubyzip#lib/zip/file.rb:499
  def check_file(path); end

  # pkg:gem/rubyzip#lib/zip/file.rb:458
  def initialize_cdir(path_or_io, buffer: T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/file.rb:503
  def on_success_replace; end

  class << self
    # Recursively adds the contents of `src_dir` to the new archive
    # `zip_file_name`, nested under `prefix` if given (the archive root
    # otherwise). `src_dir` itself is not added, only its contents.
    #
    # Symlinks are ignored (skipped, with a warning) rather than followed
    # or added, to avoid the security issues they can pose. `max_depth`
    # limits how many directory levels below `src_dir` are walked; anything
    # deeper is skipped, also with a warning (default: 16).
    #
    # pkg:gem/rubyzip#lib/zip/file.rb:191
    def add_recursive(zip_file_name, src_dir, prefix: T.unsafe(nil), max_depth: T.unsafe(nil), &continue_on_exists_proc); end

    # Count the entries in a zip archive without reading the whole set of
    # entry data into memory.
    #
    # pkg:gem/rubyzip#lib/zip/file.rb:171
    def count_entries(path_or_io); end

    # Extracts every entry in the zip archive `zip_file_name` into
    # `destination_directory`, preserving the archive's directory structure.
    #
    # pkg:gem/rubyzip#lib/zip/file.rb:199
    def extract_all(zip_file_name, destination_directory = T.unsafe(nil), &block); end

    # Iterates over the contents of the ZipFile. This is more efficient
    # than using a ZipInputStream since this methods simply iterates
    # through the entries in the central directory structure in the archive
    # whereas ZipInputStream jumps through the entire archive accessing the
    # local entry headers (which contain the same information as the
    # central directory).
    #
    # pkg:gem/rubyzip#lib/zip/file.rb:163
    def foreach(zip_file_name, &block); end

    # Similar to ::new. If a block is passed the Zip::File object is passed
    # to the block and is automatically closed afterwards, just as with
    # ruby's builtin File::open method.
    #
    # pkg:gem/rubyzip#lib/zip/file.rb:100
    def open(file_name, create: T.unsafe(nil), restore_ownership: T.unsafe(nil), restore_permissions: T.unsafe(nil), restore_times: T.unsafe(nil), compression_level: T.unsafe(nil), suppress_extra_fields: T.unsafe(nil)); end

    # Like #open, but reads zip archive contents from a String or open IO
    # stream, and outputs data to a buffer.
    # (This can be used to extract data from a
    # downloaded zip archive without first saving it to disk.)
    #
    # pkg:gem/rubyzip#lib/zip/file.rb:126
    def open_buffer(io = T.unsafe(nil), create: T.unsafe(nil), restore_ownership: T.unsafe(nil), restore_permissions: T.unsafe(nil), restore_times: T.unsafe(nil), compression_level: T.unsafe(nil), suppress_extra_fields: T.unsafe(nil)); end
  end
end

# pkg:gem/rubyzip#lib/zip/file.rb:58
Zip::File::IO_METHODS = T.let(T.unsafe(nil), Array)

# pkg:gem/rubyzip#lib/zip/file_split.rb:4
module Zip::FileSplit
  # pkg:gem/rubyzip#lib/zip/file_split.rb:13
  def get_partial_zip_file_name(zip_file_name, partial_zip_file_name); end

  # pkg:gem/rubyzip#lib/zip/file_split.rb:24
  def get_segment_count_for_split(zip_file_size, segment_size); end

  # pkg:gem/rubyzip#lib/zip/file_split.rb:9
  def get_segment_size_for_split(segment_size); end

  # pkg:gem/rubyzip#lib/zip/file_split.rb:29
  def put_split_signature(szip_file, segment_size); end

  # TODO: Make the code more understandable
  #
  # pkg:gem/rubyzip#lib/zip/file_split.rb:38
  def save_splited_part(zip_file, partial_zip_file_name, zip_file_size, szip_file_index, segment_size, segment_count); end

  # Splits an archive into parts with segment size
  #
  # pkg:gem/rubyzip#lib/zip/file_split.rb:63
  def split(zip_file_name, segment_size: T.unsafe(nil), delete_original: T.unsafe(nil), partial_zip_file_name: T.unsafe(nil)); end
end

# pkg:gem/rubyzip#lib/zip/file_split.rb:7
Zip::FileSplit::DATA_BUFFER_SIZE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/file_split.rb:5
Zip::FileSplit::MAX_SEGMENT_SIZE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/file_split.rb:6
Zip::FileSplit::MIN_SEGMENT_SIZE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/ioextras.rb:4
module Zip::IOExtras
  class << self
    # pkg:gem/rubyzip#lib/zip/ioextras.rb:8
    def copy_stream(ostream, istream); end

    # pkg:gem/rubyzip#lib/zip/ioextras.rb:12
    def copy_stream_n(ostream, istream, nbytes); end
  end
end

# Implements many of the convenience methods of IO
# such as gets, getc, read, readline and readlines
# depends on: input_finished?, produce_input and read
#
# pkg:gem/rubyzip#lib/zip/ioextras/abstract_input_stream.rb:10
module Zip::IOExtras::AbstractInputStream
  include ::ActiveSupport::ToJsonWithActiveSupportEncoder
  include ::Enumerable
  include ::Zip::IOExtras::FakeIO

  # Creates a new input stream wrapper.
  #
  # This method accepts the standard IO encoding options:
  #
  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_input_stream.rb:18
  def initialize(**opts); end

  # Calls the block with each remaining line read from the stream.
  # Does nothing if already at end-of-stream. See the Line IO
  # documentation in the IO class for more information.
  #
  # With no arguments given, reads lines as determined by line separator
  # `$/`. With only string argument `sep` given, reads lines as determined
  # by line separator `sep`. See the Line Separator documentation in the
  # IO class for more information. The two special values for `sep`
  # (`nil` and `""`) are honoured.
  #
  # With only integer argument `limit` given, limits the number of bytes
  # in each line; see the Line Limit documentation in the IO class for
  # more information.
  #
  # With arguments `sep` and `limit` given, combines the two behaviors.
  #
  # Optional keyword argument `chomp` specifies whether line separators
  # are to be omitted.
  #
  # Returns an `Enumerator` if no block is given.
  #
  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_input_stream.rb:199
  def each(sep = T.unsafe(nil), limit = T.unsafe(nil), chomp: T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_input_stream.rb:207
  def each_line(sep = T.unsafe(nil), limit = T.unsafe(nil), chomp: T.unsafe(nil)); end

  # Alias for compatibility. Remove for version 4.
  #
  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_input_stream.rb:217
  def eof; end

  # Returns `true` if the stream is positioned at its end, `false`
  # otherwise. See Position documentation in the IO class for more
  # information.
  #
  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_input_stream.rb:212
  def eof?; end

  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_input_stream.rb:164
  def flush; end

  # Reads and returns a line from the stream. See the Line IO
  # documentation in the IO class for more information.
  #
  # With no arguments given, returns the next line as determined by line
  # separator `$/`, or `nil` if none.
  #
  # With only string argument `sep` given, returns the next line as
  # determined by line separator `sep`, or `nil` if none. See the
  # Line Separator documentation in the IO class for more information.
  # The two special values for `sep` (`nil` and `""`) are honoured.
  #
  # With only integer argument `limit` given, limits the number of bytes
  # in the line; see the Line Limit documentation in the IO class for more
  # information.
  #
  # With arguments `sep` and `limit` given, combines the two behaviors.
  #
  # Optional keyword argument `chomp` specifies whether line separators
  # are to be omitted.
  #
  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_input_stream.rb:118
  def gets(sep = T.unsafe(nil), limit = T.unsafe(nil), chomp: T.unsafe(nil)); end

  # Returns (or sets) the current line number in the decompressed
  # (possibly decrypted) data stream. See the Line Number documentation
  # for the IO class for more information.
  #
  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_input_stream.rb:28
  def lineno; end

  # Returns (or sets) the current line number in the decompressed
  # (possibly decrypted) data stream. See the Line Number documentation
  # for the IO class for more information.
  #
  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_input_stream.rb:28
  def lineno=(_arg0); end

  # Returns the current position (in bytes) in the decompressed (possibly
  # decrypted) data stream.
  #
  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_input_stream.rb:32
  def pos; end

  # Reads bytes from the stream decompressed (possibly decrypted) data
  # stream. If `maxlen` is `nil`, reads all bytes; otherwise, reads up to
  # `maxlen` bytes. If `maxlen` is zero, returns an empty string.
  #
  # Returns a string (either a new string or the given `out_string`)
  # containing the bytes read. The string's encoding is the unchanged
  # encoding of `out_string`, if `out_string` is given; `ASCII-8BIT`,
  # otherwise.
  #
  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_input_stream.rb:42
  def read(maxlen = T.unsafe(nil), out_string = T.unsafe(nil)); end

  # Reads a line as with #gets, but raises `EOFError` if already at
  # end-of-stream.
  #
  # Optional keyword argument `chomp` specifies whether line separators
  # are to be omitted.
  #
  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_input_stream.rb:173
  def readline(sep = T.unsafe(nil), limit = T.unsafe(nil), chomp: T.unsafe(nil)); end

  # Reads and returns all remaining lines from the stream. See the Line IO
  # documentation in the IO class for more information.
  #
  # With no arguments given, returns lines as determined by line
  # separator `$/`, or `nil` if none.
  #
  # With only string argument `sep` given, returns lines as
  # determined by line separator `sep`, or `nil` if none. See the
  # Line Separator documentation in the IO class for more information.
  # The two special values for `sep` (`nil` and `""`) are honoured.
  #
  # With only integer argument `limit` given, limits the number of bytes
  # in each line; see the Line Limit documentation in the IO class for more
  # information.
  #
  # With arguments `sep` and `limit` given, combines the two behaviors.
  #
  # Optional keyword argument `chomp` specifies whether line separators
  # are to be omitted.
  #
  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_input_stream.rb:95
  def readlines(sep = T.unsafe(nil), limit = T.unsafe(nil), chomp: T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_input_stream.rb:160
  def ungetc(byte); end
end

# Implements many of the output convenience methods of IO.
# relies on <<
#
# pkg:gem/rubyzip#lib/zip/ioextras/abstract_output_stream.rb:9
module Zip::IOExtras::AbstractOutputStream
  include ::Zip::IOExtras::FakeIO

  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_output_stream.rb:17
  def print(*params); end

  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_output_stream.rb:28
  def printf(a_format_string, *params); end

  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_output_stream.rb:33
  def putc(an_object); end

  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_output_stream.rb:45
  def puts(*params); end

  # pkg:gem/rubyzip#lib/zip/ioextras/abstract_output_stream.rb:12
  def write(data); end
end

# pkg:gem/rubyzip#lib/zip/ioextras.rb:5
Zip::IOExtras::CHUNK_SIZE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/ioextras/fake_io.rb:5
module Zip::IOExtras::FakeIO
  # pkg:gem/rubyzip#lib/zip/ioextras/fake_io.rb:8
  def initialize(**opts); end

  # pkg:gem/rubyzip#lib/zip/ioextras/fake_io.rb:6
  def external_encoding; end

  # pkg:gem/rubyzip#lib/zip/ioextras/fake_io.rb:6
  def internal_encoding; end

  # Implement kind_of? in order to pretend to be an IO object.
  #
  # pkg:gem/rubyzip#lib/zip/ioextras/fake_io.rb:34
  def kind_of?(object); end

  # pkg:gem/rubyzip#lib/zip/ioextras/fake_io.rb:19
  def set_encoding(ext_enc, int_enc = T.unsafe(nil)); end
end

# pkg:gem/rubyzip#lib/zip/inflater.rb:4
class Zip::Inflater < ::Zip::Decompressor
  # pkg:gem/rubyzip#lib/zip/inflater.rb:5
  def initialize(*args); end

  # Alias for compatibility. Remove for version 4.
  #
  # pkg:gem/rubyzip#lib/zip/inflater.rb:29
  def eof; end

  # pkg:gem/rubyzip#lib/zip/inflater.rb:24
  def eof?; end

  # pkg:gem/rubyzip#lib/zip/inflater.rb:12
  def read(maxlen = T.unsafe(nil)); end

  private

  # pkg:gem/rubyzip#lib/zip/inflater.rb:47
  def input_finished?; end

  # pkg:gem/rubyzip#lib/zip/inflater.rb:33
  def produce_input; end
end

# InputStream is the basic class for reading zip entries in a
# zip file. It is possible to create a InputStream object directly,
# passing the zip file name to the constructor, but more often than not
# the InputStream will be obtained from a File (perhaps using the
# ZipFileSystem interface) object for a particular entry in the zip
# archive.
#
# A InputStream inherits IOExtras::AbstractInputStream in order
# to provide an IO-like interface for reading from a single zip
# entry. Beyond methods for mimicking an IO-object it contains
# the method get_next_entry for iterating through the entries of
# an archive. get_next_entry returns a Entry object that describes
# the zip entry the InputStream is currently reading from.
#
# Example that creates a zip archive with ZipOutputStream and reads it
# back again with a InputStream.
#
#   require 'zip'
#
#   Zip::OutputStream.open("my.zip") do |io|
#
#     io.put_next_entry("first_entry.txt")
#     io.write "Hello world!"
#
#     io.put_next_entry("adir/first_entry.txt")
#     io.write "Hello again!"
#   end
#
#
#   Zip::InputStream.open("my.zip") do |io|
#
#     while (entry = io.get_next_entry)
#       puts "Contents of #{entry.name}: '#{io.read}'"
#     end
#   end
#
# java.util.zip.ZipInputStream is the original inspiration for this
# class.
#
# pkg:gem/rubyzip#lib/zip/input_stream.rb:43
class Zip::InputStream
  include ::Enumerable
  include ::Zip::IOExtras::FakeIO
  include ::Zip::IOExtras::AbstractInputStream

  # Opens the indicated zip file. An exception is thrown
  # if the specified offset in the specified filename is
  # not a local zip entry header.
  #
  # @param context [String||IO||StringIO] file path or IO/StringIO object
  # @param offset [Integer] offset in the IO/StringIO
  # This method also accepts the standard IO encoding options:
  #
  # pkg:gem/rubyzip#lib/zip/input_stream.rb:56
  def initialize(context, offset: T.unsafe(nil), decrypter: T.unsafe(nil), **opts); end

  # Close this InputStream. All further IO will raise an IOError.
  #
  # pkg:gem/rubyzip#lib/zip/input_stream.rb:66
  def close; end

  # Returns an Entry object and positions the stream at the beginning of
  # the entry data. It is necessary to call this method on a newly created
  # InputStream before reading from the first entry in the archive.
  # Returns nil when there are no more entries.
  #
  # pkg:gem/rubyzip#lib/zip/input_stream.rb:74
  def get_next_entry; end

  # Rewinds the stream to the beginning of the current entry.
  #
  # pkg:gem/rubyzip#lib/zip/input_stream.rb:85
  def rewind; end

  # Returns the size of the current entry, or `nil` if there isn't one.
  #
  # pkg:gem/rubyzip#lib/zip/input_stream.rb:120
  def size; end

  # Modelled after IO#sysread.
  #
  # Reads up to maxlen bytes from the stream; returns a string
  # (either a new string or the given out_string).
  # Its encoding is the unchanged encoding of out_string, if out_string is
  # given; ASCII-8BIT, otherwise. Output contains maxlen bytes from the
  # stream, if available; otherwise contains all available bytes, if any
  # available; otherwise is an empty string.
  #
  # This method should not be used with buffered input stream-reader methods,
  # such as #read, #readline, #gets.
  #
  # pkg:gem/rubyzip#lib/zip/input_stream.rb:105
  def sysread(maxlen = T.unsafe(nil), out_string = T.unsafe(nil)); end

  protected

  # pkg:gem/rubyzip#lib/zip/input_stream.rb:169
  def assemble_io; end

  # pkg:gem/rubyzip#lib/zip/input_stream.rb:201
  def get_decompressor(io); end

  # pkg:gem/rubyzip#lib/zip/input_stream.rb:181
  def get_decrypted_io; end

  # pkg:gem/rubyzip#lib/zip/input_stream.rb:144
  def get_io(io_or_file, offset = T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/input_stream.rb:226
  def input_finished?; end

  # pkg:gem/rubyzip#lib/zip/input_stream.rb:156
  def open_entry; end

  # pkg:gem/rubyzip#lib/zip/input_stream.rb:222
  def produce_input(maxlen = T.unsafe(nil)); end

  class << self
    # Same as #initialize but if a block is passed the opened
    # stream is passed to the block and closed when the block
    # returns.
    #
    # pkg:gem/rubyzip#lib/zip/input_stream.rb:130
    def open(filename_or_io, offset: T.unsafe(nil), decrypter: T.unsafe(nil), **opts); end
  end
end

# pkg:gem/rubyzip#lib/zip/input_stream.rb:44
Zip::InputStream::CHUNK_SIZE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:11
Zip::LOCAL_ENTRY_SIGNATURE = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:12
Zip::LOCAL_ENTRY_STATIC_HEADER_LENGTH = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:13
Zip::LOCAL_ENTRY_TRAILING_DESCRIPTOR_LENGTH = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/null_compressor.rb:4
class Zip::NullCompressor < ::Zip::Compressor
  include ::Singleton::SingletonInstanceMethods
  include ::Singleton
  extend ::Singleton::SingletonClassMethods

  # pkg:gem/rubyzip#lib/zip/null_compressor.rb:7
  def <<(_data); end

  # pkg:gem/rubyzip#lib/zip/null_compressor.rb:11
  def compressed_size; end

  # pkg:gem/rubyzip#lib/zip/null_compressor.rb:11
  def size; end

  class << self
    private

    # pkg:gem/rubyzip#lib/zip/null_compressor.rb:5
    def allocate; end

    # pkg:gem/rubyzip#lib/zip/null_compressor.rb:5
    def new(*_arg0); end
  end
end

# pkg:gem/rubyzip#lib/zip/null_decompressor.rb:4
module Zip::NullDecompressor
  private

  # Alias for compatibility. Remove for version 4.
  #
  # pkg:gem/rubyzip#lib/zip/null_decompressor.rb:16
  def eof; end

  # pkg:gem/rubyzip#lib/zip/null_decompressor.rb:11
  def eof?; end

  # pkg:gem/rubyzip#lib/zip/null_decompressor.rb:7
  def read(_length = T.unsafe(nil), _outbuf = T.unsafe(nil)); end

  class << self
    # pkg:gem/rubyzip#lib/zip/null_decompressor.rb:11
    def eof?; end

    # pkg:gem/rubyzip#lib/zip/null_decompressor.rb:7
    def read(_length = T.unsafe(nil), _outbuf = T.unsafe(nil)); end
  end
end

# pkg:gem/rubyzip#lib/zip/crypto/null_encryption.rb:14
class Zip::NullEncrypter < ::Zip::Encrypter
  include ::Zip::NullEncryption

  # pkg:gem/rubyzip#lib/zip/crypto/null_encryption.rb:25
  def data_descriptor(_crc32, _compressed_size, _uncompressed_size); end

  # pkg:gem/rubyzip#lib/zip/crypto/null_encryption.rb:21
  def encrypt(data); end

  # pkg:gem/rubyzip#lib/zip/crypto/null_encryption.rb:17
  def header(_mtime); end

  # pkg:gem/rubyzip#lib/zip/crypto/null_encryption.rb:29
  def reset!; end
end

# pkg:gem/rubyzip#lib/zip/crypto/null_encryption.rb:4
module Zip::NullEncryption
  # pkg:gem/rubyzip#lib/zip/crypto/null_encryption.rb:9
  def gp_flags; end

  # pkg:gem/rubyzip#lib/zip/crypto/null_encryption.rb:5
  def header_bytesize; end
end

# pkg:gem/rubyzip#lib/zip/null_input_stream.rb:4
module Zip::NullInputStream
  include ::Zip::NullDecompressor
  include ::ActiveSupport::ToJsonWithActiveSupportEncoder
  include ::Enumerable
  include ::Zip::IOExtras::FakeIO
  include ::Zip::IOExtras::AbstractInputStream
end

# ZipOutputStream is the basic class for writing zip files. It is
# possible to create a ZipOutputStream object directly, passing
# the zip file name to the constructor, but more often than not
# the ZipOutputStream will be obtained from a ZipFile (perhaps using the
# ZipFileSystem interface) object for a particular entry in the zip
# archive.
#
# A ZipOutputStream inherits IOExtras::AbstractOutputStream in order
# to provide an IO-like interface for writing to a single zip
# entry. Beyond methods for mimicking an IO-object it contains
# the method put_next_entry that closes the current entry
# and creates a new.
#
# Please refer to ZipInputStream for example code.
#
# java.util.zip.ZipOutputStream is the original inspiration for this
# class.
#
# pkg:gem/rubyzip#lib/zip/output_stream.rb:24
class Zip::OutputStream
  include ::Zip::IOExtras::FakeIO
  include ::Zip::IOExtras::AbstractOutputStream
  extend ::Forwardable

  # Opens the indicated zip file. If a file with that name already
  # exists it will be overwritten.
  #
  # pkg:gem/rubyzip#lib/zip/output_stream.rb:32
  def initialize(file_name, stream: T.unsafe(nil), encrypter: T.unsafe(nil), suppress_extra_fields: T.unsafe(nil), **opts); end

  # Modeled after IO.<<
  #
  # pkg:gem/rubyzip#lib/zip/output_stream.rb:208
  def <<(data); end

  # Closes the stream and writes the central directory to the zip file
  #
  # pkg:gem/rubyzip#lib/zip/output_stream.rb:83
  def close; end

  # Closes the stream and writes the central directory to the zip file
  #
  # pkg:gem/rubyzip#lib/zip/output_stream.rb:94
  def close_buffer; end

  # pkg:gem/rubyzip#lib/zip/output_stream.rb:28
  def comment(*, **, &); end

  # pkg:gem/rubyzip#lib/zip/output_stream.rb:28
  def comment=(*, **, &); end

  # pkg:gem/rubyzip#lib/zip/output_stream.rb:127
  def copy_raw_entry(entry); end

  # Closes the current entry and opens a new for writing.
  # +entry+ can be a ZipEntry object or a string.
  #
  # pkg:gem/rubyzip#lib/zip/output_stream.rb:107
  def put_next_entry(entry_name, comment = T.unsafe(nil), extra = T.unsafe(nil), compression_method = T.unsafe(nil), level = T.unsafe(nil)); end

  protected

  # pkg:gem/rubyzip#lib/zip/output_stream.rb:201
  def finish; end

  private

  # pkg:gem/rubyzip#lib/zip/output_stream.rb:148
  def finalize_current_entry; end

  # pkg:gem/rubyzip#lib/zip/output_stream.rb:178
  def get_compressor(entry); end

  # pkg:gem/rubyzip#lib/zip/output_stream.rb:168
  def init_next_entry(entry); end

  # pkg:gem/rubyzip#lib/zip/output_stream.rb:189
  def update_local_headers; end

  class << self
    # Same as #initialize but if a block is passed the opened
    # stream is passed to the block and closed when the block
    # returns.
    #
    # pkg:gem/rubyzip#lib/zip/output_stream.rb:55
    def open(file_name, encrypter: T.unsafe(nil), suppress_extra_fields: T.unsafe(nil), **opts); end

    # Same as #open but writes to a filestream instead
    #
    # pkg:gem/rubyzip#lib/zip/output_stream.rb:73
    def write_buffer(io = T.unsafe(nil), encrypter: T.unsafe(nil), suppress_extra_fields: T.unsafe(nil), **opts); end
  end
end

# pkg:gem/rubyzip#lib/zip/pass_thru_compressor.rb:4
class Zip::PassThruCompressor < ::Zip::Compressor
  # pkg:gem/rubyzip#lib/zip/pass_thru_compressor.rb:5
  def initialize(output_stream, encrypter = T.unsafe(nil)); end

  # pkg:gem/rubyzip#lib/zip/pass_thru_compressor.rb:13
  def <<(data); end

  # pkg:gem/rubyzip#lib/zip/pass_thru_compressor.rb:20
  def crc; end

  # pkg:gem/rubyzip#lib/zip/pass_thru_compressor.rb:20
  def size; end
end

# pkg:gem/rubyzip#lib/zip/pass_thru_decompressor.rb:4
class Zip::PassThruDecompressor < ::Zip::Decompressor
  # pkg:gem/rubyzip#lib/zip/pass_thru_decompressor.rb:5
  def initialize(*args); end

  # Alias for compatibility. Remove for version 4.
  #
  # pkg:gem/rubyzip#lib/zip/pass_thru_decompressor.rb:26
  def eof; end

  # pkg:gem/rubyzip#lib/zip/pass_thru_decompressor.rb:21
  def eof?; end

  # pkg:gem/rubyzip#lib/zip/pass_thru_decompressor.rb:10
  def read(maxlen = T.unsafe(nil)); end
end

# :stopdoc:
#
# pkg:gem/rubyzip#lib/zip/constants.rb:6
Zip::RUNNING_ON_WINDOWS = T.let(T.unsafe(nil), T.untyped)

# pkg:gem/rubyzip#lib/zip/constants.rb:19
Zip::SPLIT_FILE_SIGNATURE = T.let(T.unsafe(nil), Integer)

# Error raised if a split archive is read. Rubyzip does not support reading
# split archives.
#
# pkg:gem/rubyzip#lib/zip/errors.rb:130
class Zip::SplitArchiveError < ::Zip::Error
  # The message returned by this error.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:132
  def message; end
end

# pkg:gem/rubyzip#lib/zip/streamable_directory.rb:4
class Zip::StreamableDirectory < ::Zip::Entry
  # pkg:gem/rubyzip#lib/zip/streamable_directory.rb:5
  def initialize(zipfile, entry, src_path = T.unsafe(nil), permission = T.unsafe(nil)); end
end

# pkg:gem/rubyzip#lib/zip/streamable_stream.rb:4
class Zip::StreamableStream
  # pkg:gem/rubyzip#lib/zip/streamable_stream.rb:5
  def initialize(entry); end

  # pkg:gem/rubyzip#lib/zip/streamable_stream.rb:46
  def clean_up; end

  # pkg:gem/rubyzip#lib/zip/streamable_stream.rb:23
  def get_input_stream; end

  # pkg:gem/rubyzip#lib/zip/streamable_stream.rb:11
  def get_output_stream; end

  # pkg:gem/rubyzip#lib/zip/streamable_stream.rb:41
  def write_to_zip_output_stream(output_stream); end
end

# Error raised if there is not enough metadata for the entry to be streamed.
#
# pkg:gem/rubyzip#lib/zip/errors.rb:138
class Zip::StreamingError < ::Zip::Error
  # Create a new StreamingError with the specified entry.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:143
  def initialize(entry); end

  # The entry that has caused this error.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:140
  def entry; end

  # The message returned by this error.
  #
  # pkg:gem/rubyzip#lib/zip/errors.rb:149
  def message; end
end

# pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:77
class Zip::TraditionalDecrypter < ::Zip::Decrypter
  include ::Zip::TraditionalEncryption

  # pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:91
  def check_integrity!(_io); end

  # pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:80
  def decrypt(data); end

  # pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:84
  def reset!(header); end

  private

  # pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:95
  def decode(num); end
end

# pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:43
class Zip::TraditionalEncrypter < ::Zip::Encrypter
  include ::Zip::TraditionalEncryption

  # pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:60
  def data_descriptor(crc32, compressed_size, uncompressed_size); end

  # pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:56
  def encrypt(data); end

  # pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:46
  def header(mtime); end

  # pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:64
  def reset!; end

  private

  # pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:70
  def encode(num); end
end

# pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:6
module Zip::TraditionalEncryption
  # pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:7
  def initialize(password); end

  # pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:16
  def gp_flags; end

  # pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:12
  def header_bytesize; end

  protected

  # pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:37
  def decrypt_byte; end

  # pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:22
  def reset_keys!; end

  # pkg:gem/rubyzip#lib/zip/crypto/traditional_encryption.rb:31
  def update_keys(num); end
end

# The version of the Rubyzip library.
#
# pkg:gem/rubyzip#lib/zip/version.rb:5
Zip::VERSION = T.let(T.unsafe(nil), String)

# pkg:gem/rubyzip#lib/zip/constants.rb:14
Zip::VERSION_MADE_BY = T.let(T.unsafe(nil), Integer)

# this library's version
#
# pkg:gem/rubyzip#lib/zip/constants.rb:15
Zip::VERSION_NEEDED_TO_EXTRACT = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:17
Zip::VERSION_NEEDED_TO_EXTRACT_AES = T.let(T.unsafe(nil), Integer)

# pkg:gem/rubyzip#lib/zip/constants.rb:16
Zip::VERSION_NEEDED_TO_EXTRACT_ZIP64 = T.let(T.unsafe(nil), Integer)

# Remove this when JRuby#3962 is fixed.
#
# pkg:gem/rubyzip#lib/zip.rb:68
Zip::ZLIB_FLUSHING_STRATEGY = T.let(T.unsafe(nil), Integer)
