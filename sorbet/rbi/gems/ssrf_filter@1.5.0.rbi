# typed: true

# DO NOT EDIT MANUALLY
# This is an autogenerated file for types exported from the `ssrf_filter` gem.
# Please instead update this file by running `bin/tapioca gem ssrf_filter`.


# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#8
class SsrfFilter
  class << self
    # source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#128
    def delete(url, options = T.unsafe(nil), &block); end

    # source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#128
    def get(url, options = T.unsafe(nil), &block); end

    # source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#128
    def head(url, options = T.unsafe(nil), &block); end

    # source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#128
    def patch(url, options = T.unsafe(nil), &block); end

    # source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#128
    def post(url, options = T.unsafe(nil), &block); end

    # source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#128
    def put(url, options = T.unsafe(nil), &block); end

    private

    # @return [Boolean]
    #
    # source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#195
    def different_origin?(uri1, uri2); end

    # source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#209
    def fetch_once(uri, ip, verb, options, &block); end

    # @return [Boolean]
    #
    # source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#190
    def ipaddr_has_mask?(ipaddr); end

    # source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#26
    def ipv4_from_rfc6052(ipv6_addr, prefix_len); end

    # source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#199
    def normalized_hostname(uri); end

    # @raise [ArgumentError]
    #
    # source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#9
    def prefixlen_from_ipaddr(ipaddr); end

    # @return [Boolean]
    #
    # source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#170
    def unsafe_ip_address?(ip_address); end

    # source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#263
    def validate_request(request); end
  end
end

# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#121
class SsrfFilter::CRLFInjection < ::SsrfFilter::Error; end

# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#124
class SsrfFilter::CredentialLeakage < ::SsrfFilter::Error; end

# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#92
SsrfFilter::DEFAULT_ALLOW_UNFOLLOWED_REDIRECTS = T.let(T.unsafe(nil), FalseClass)

# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#93
SsrfFilter::DEFAULT_MAX_REDIRECTS = T.let(T.unsafe(nil), Integer)

# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#95
SsrfFilter::DEFAULT_ON_CROSS_ORIGIN_REDIRECT = T.let(T.unsafe(nil), Symbol)

# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#88
SsrfFilter::DEFAULT_RESOLVER = T.let(T.unsafe(nil), Proc)

# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#86
SsrfFilter::DEFAULT_SCHEME_WHITELIST = T.let(T.unsafe(nil), Array)

# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#94
SsrfFilter::DEFAULT_SENSITIVE_HEADERS = T.let(T.unsafe(nil), Array)

# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#106
class SsrfFilter::Error < ::StandardError; end

# https://en.wikipedia.org/wiki/Reserved_IP_addresses
#
# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#40
SsrfFilter::IPV4_BLACKLIST = T.let(T.unsafe(nil), Array)

# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#61
SsrfFilter::IPV6_BLACKLIST = T.let(T.unsafe(nil), Array)

# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#109
class SsrfFilter::InvalidUriScheme < ::SsrfFilter::Error; end

# NAT64 local-use prefix (RFC 8215), uses RFC 6052 /48 encoding (checked at runtime).
#
# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#59
SsrfFilter::NAT64_LOCAL_PREFIX = T.let(T.unsafe(nil), IPAddr)

# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#112
class SsrfFilter::PrivateIPAddress < ::SsrfFilter::Error; end

# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#118
class SsrfFilter::TooManyRedirects < ::SsrfFilter::Error; end

# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#115
class SsrfFilter::UnresolvedHostname < ::SsrfFilter::Error; end

# source://ssrf_filter//lib/ssrf_filter/ssrf_filter.rb#97
SsrfFilter::VERB_MAP = T.let(T.unsafe(nil), Hash)

# source://ssrf_filter//lib/ssrf_filter/version.rb#4
SsrfFilter::VERSION = T.let(T.unsafe(nil), String)
