...
RSpec.describe Mcp::Tools::UpdateWhitelistTool do
  describe "class methods" do
    it "has the correct tool name" do
      expect(described_class.tool_name).to eq("update_whitelist")
    end
  ...
  describe "#execute" do
    ...
    let!(:whitelist) { create(:whitelist, file: "test_whitelist", hidden: false) }

    it "updates hidden status by id" do
      result = tool.execute(id: whitelist.id, hidden: true)
    ...
    it "updates hidden status by file name" do
      result = tool.execute(file: "test_whitelist", hidden: true)
    ...
    it "finds whitelist case-insensitively" do
      result = tool.execute(file: "TEST_WHITELIST", hidden: true)
    ...
    it "can unhide a hidden whitelist" do
      whitelist.update!(hidden: true)

      result = tool.execute(id: whitelist.id, hidden: false)

      expect(result[:success]).to be true
      expect(result[:whitelist][:hidden]).to be false

      whitelist.reload
      expect(whitelist.hidden).to be false
    end

    it "fails if whitelist not found by id" do
      result = tool.execute(id: 999999, hidden: true)

      expect(result[:success]).to be false
      expect(result[:error]).to eq("Whitelist not found")
    end

    it "fails if whitelist not found by file" do
    ...
    it "fails if hidden parameter not provided" do
      result = tool.execute(id: whitelist.id)
